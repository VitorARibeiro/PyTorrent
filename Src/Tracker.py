...
def SendRequest(file_path):
    Decoded_Torrent = TorrentParser.DecodedTorrent(file_path)
    ...
    Encoded_Info = TorrentParser.BencodedInfo(Decoded_Torrent)
    info_hash = Sha1.hash_sha1(Encoded_Info)

    peer_id = '-PC0001-' + \
        ''.join([str(random.randint(0, 9)) for _ in range(12)])

    left = TorrentParser.SumFileLengths(Decoded_Torrent)
